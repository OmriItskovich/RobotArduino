...
void calculate_motor_speeds(int xMove, int yMove){
  ...
  Serial.print(xMove);
  Serial.print(",Y_Move,");
  Serial.print(yMove/100.0);

  float v=(yMove/100.0)*max_linear_speed;
  float omega = (xMove/100.0) * max_angular_speed;
  ...
  float bSpeed=v+omega;
  float aSpeed=v-omega;
  ...
  if(xMove==0.0&&yMove==0.0)
    ...
    analogWrite(PWM_A,aSpeed); //right
    analogWrite(PWM_B,bSpeed); //left
  }
